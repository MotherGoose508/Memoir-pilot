...
export default function Home() {
  ...
  const correct = selected !== null && (typed ? normalize(selected) === normalize(card.term) : selected === card.definition);
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      } else if (!typed && !selected && /^[1-4]$/.test(event.key)) {
        setSelected(card.options[Number(event.key) - 1]);
      }
    };
    window.addEventListener("keydown", onKeyDown);
    return () => window.removeEventListener("keydown", onKeyDown);
  }, [card.options, correct, dialog, index, selected, sessionEnded, typed, view]);
